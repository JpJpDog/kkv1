...
impl Drop for PageManager2 {
    fn drop(&mut self) {
        ...
        unsafe {
            drop(Box::from_raw(self.tmp_map.load(Ordering::SeqCst)));
            drop(Box::from_raw(self.logging_map.load(Ordering::SeqCst)));
