...
use self::palm_msg::{FindReqs, FindRsps, DataReqs, DataRsps, InnerReqs, InnerRsps, PALMCmd, PALMResult};
use self::palm_worker::PALMWorker;
...
impl<
        K: 'static + Ord + Send + Sync + Clone + Default,
        V: 'static + Clone + Send + Sync + Default,
    > PALMTree<K, V>
{
    ...
    fn merge_find_rsp(
        find_rsps: &[Vec<Vec<(PageId, usize)>>],
        depth: usize,
    ) -> Vec<(PageId, usize)> {
